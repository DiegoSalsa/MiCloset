-- Migración: Agregar campos de estilo y temporada a garments
-- Fecha: 2025-12-22
-- Descripción: Agrega campos para mejorar el sistema de matching inteligente
...
ADD COLUMN IF NOT EXISTS season VARCHAR(50) CHECK (season IN ('primavera', 'verano', 'otoño', 'invierno', 'todo_ano'));
...
CREATE TABLE IF NOT EXISTS outfit_ratings (
    ...
    garment_ids UUID[] NOT NULL, -- Array de IDs de prendas del outfit
    occasion VARCHAR(100),
    weather VARCHAR(20),
    rating BOOLEAN NOT NULL, -- true = 👍 gustó, false = 👎 no gustó
...
-- Trigger para actualizar garments
CREATE TRIGGER IF NOT EXISTS update_garments_updated_at BEFORE UPDATE ON garments
